...
khanza.post("/daftarBaruPasienLama", (req, res) => {
    ...
    let tanggal = req.body.send.tgl_periksa
  ...
  if(utils.timestamp_checker(message._data.id.remote.split('@')[0],message.timestamp)){   
    ...
    //menu cara 
      ...
      if(fil.array_search(utils.temp_state_user,message._data.id.remote)[2]===2){
        ...
        let dataDaftar = {
          'id_daftar':message._data.id.remote.split('@')[0]+''+utils.removeSpasi(result[2].split(":")[1])+''+utils.getDateToday(),
          'Nama':utils.removeSpasi(result[1].split(":")[1]),
          'NIK':utils.removeSpasi(result[2].split(":")[1]),
          'Alamat':utils.removeSpasi(result[3].split(":")[1]),
          'Jenis_Bayar':utils.removeSpasi(result[4].split(":")[1]),
          'Poli_tujuan':utils.removeSpasi(result[5].split(":")[1])+'_'+utils.removeSpasi(result[6].split(":")[1]),
          'no_wa':message._data.id.remote.split('@')[0],
          'nama_penjamin':utils.removeSpasi(result[7].split(":")[1]),
          'nama_ibu':utils.removeSpasi(result[8].split(":")[1]),
          'no_penjamin':utils.removeSpasi(result[9].split(":")[1]),
        }
